...
namespace GlobalSnowEffect {
    ...
    public class GlobalSnowEditor : Editor {
        ...
        SerializedProperty bigWorldAntiflicker;
        ...
        SerializedProperty defaultExclusionLayer, exclusionDoubleSided, exclusionDefaultCutOff, exclusionUseFastMaskShader, coverageMask, coverageMaskTexture, coverageMaskWorldSize, coverageMaskWorldCenter, groundCoverage, coverageUpdateMethod, coverageDepthDebug;
        ...
        void OnEnable() {
            ...
            bigWorldAntiflicker = serializedObject.FindProperty("_bigWorldAntiflicker");
        ...
        public override void OnInspectorGUI() {
            ...
            if (expandSection[SCENE_SETTINGS]) {
                ...
                EditorGUILayout.PropertyField(bigWorldAntiflicker, new GUIContent("Big World Antiflicker", "Enables camera relative rendering to avoid flickering in large worlds."));
            ...
            if (expandSection[COVERAGE_SETTINGS]) {
                ...
                if (coverageMask.boolValue) {
                    EditorGUILayout.PropertyField(coverageMaskTexture, new GUIContent("   Texture (R)", "Snow coverage mask. Red channel is used. A value of zero means no snow."));
                ...
                if (coverageUpdateMethod.intValue == (int)SnowCoverageUpdateMode.Disabled) {
            ...
            if (expandSection[FEATURE_SETTINGS]) {
                if (footprints.boolValue || terrainMarks.boolValue) {
                    if (!deferred && distanceOptimization.boolValue && detailDistance.floatValue <= 0) {
                        ...
                        GUI.enabled = false;
                    } else if (coverageUpdateMethod.intValue == (int)SnowCoverageUpdateMode.Disabled) {
            ...
            if (targets.Length == 1) {
                ...
                if (expandSection[MASK_EDITOR]) {
                    ...
                    if (maskEditorEnabled.boolValue) {
                        ...
                        if (tex != null) {
                            if (currentMaskTexture != tex && tex.isReadable) {
        ...
        private void PaintOnMaskPosition(Vector3 pos) {
            ...
            byte color = maskBrushMode.intValue == (int)MaskTextureBrushMode.AddSnow ? (byte)255 : (byte)0;
            ...
            for (int j = ty - brushSize; j < ty + brushSize; j++) {
                ...
                for (int k = tx - brushSize; k < tx + brushSize; k++) {
                    ...
                    if (op <= 1f && threshold * op < fuzziness) {
                        maskColors[jj + k].r = (byte)(colort + maskColors[jj + k].r * brushOpacity);
                    }
                }
            }
