...
namespace GlobalSnowEffect {
    ...
    public enum SnowCoverageUpdateMode {
        EveryFrame,
        Discrete,
        Manual,
        Disabled
    ...
    public enum DeferredCameraEvent {
        BeforeReflections = 0,
        BeforeLighting = 1
    }
    ...
    static class CameraEventExtensions {
        public static CameraEvent ToUnityCameraEvent(this DeferredCameraEvent e) {
            switch (e) {
                case DeferredCameraEvent.BeforeLighting:
    ...
    public partial class GlobalSnow : MonoBehaviour {
        ...
        [SerializeField]
        bool _forceForwardRenderingPath;
        ...
        [SerializeField]
        DeferredCameraEvent _deferredCameraEvent = DeferredCameraEvent.BeforeReflections;

        public DeferredCameraEvent deferredCameraEvent {
        ...
        [SerializeField]
        bool _bigWorldAntiflicker = true;

        public bool bigWorldAntiflicker {
            get { return _bigWorldAntiflicker; }
            set {
                if (value != _bigWorldAntiflicker) {
                    _bigWorldAntiflicker = value;
                    UpdateMaterialProperties();
                }
            }
        }
        ...
        [SerializeField]
        SnowCoverageUpdateMode _coverageUpdateMethod = SnowCoverageUpdateMode.Discrete;

        public SnowCoverageUpdateMode coverageUpdateMethod {
        ...
        Mesh decalMesh;
        DeferredCameraEvent currentCameraEvent = DeferredCameraEvent.BeforeReflections;
        ...
        readonly List<Terrain> activeTerrains = new List<Terrain>();
#endif
        ...
        void RebuildCommandBuffer() {
            ...
            // Copy normals
            bool flatShading = _snowQuality == SnowQuality.FlatShading;
            if (!flatShading) {
        ...
        void UpdateSnowCoverageNow() {
            ...
            if (depthTexture == null) {
                depthTexture = new RenderTexture(res, res, 24, RenderTextureFormat.RFloat, RenderTextureReadWrite.Linear);
        ...
        void SnowRender_ForwardPath() {
            ...
            if (_snowQuality == SnowQuality.ReliefMapping) {
                ...
                int terrainsCount = activeTerrains.Count;
#else
                Terrain[] activeTerrains = Terrain.activeTerrains;
                int terrainsCount = activeTerrains.Length;
#endif
                for (int k = 0; k < terrainsCount; k++) {
                    activeTerrains[k].SetSplatMaterialPropertyBlock(terrainMatPropBlock);
                }
            }
        ...
        void OnPreCull() {
            ...
            if (_coverageUpdateMethod != SnowCoverageUpdateMode.Disabled) {
                if (depthTexture == null || !depthTexture.IsCreated()) {
                    CleanUpTextureDepth();
                    needUpdateSnowCoverage = true;
                }

                updateCoverage = !Application.isPlaying || needUpdateSnowCoverage || _coverageUpdateMethod == SnowCoverageUpdateMode.EveryFrame;
                if (!updateCoverage && _coverageUpdateMethod == SnowCoverageUpdateMode.Discrete) {
        ...
        private void OnPreRender() {
            if (cbMat != null && cameraEffect != null) {
                if (!_bigWorldAntiflicker || UnityEngine.XR.XRSettings.enabled) {
        ...
        public void MaskPaint(Vector3 pos, byte value, float brushWidth, float brushOpacity = 1f, float brushFuzziness = 0f, MaskPaintMethod paintMethod = MaskPaintMethod.BackgroundThread) {
            ...
            if (paintMethod == MaskPaintMethod.BackgroundThread && Application.isPlaying) {
                ...
                return;
            } else if (paintMethod == MaskPaintMethod.GPU) {
