    ...
    sampler2D_float _GS_FootprintTex;
	...
	float4    _GS_SnowData2;	// x = minimum altitude for vegetation and trees, y = altitude scattering, z = coverage extension, w = billboard coverage
	...
	float4    _GS_SnowData5;    // x = slope threshold, y = slope sharpness, z = slope noise, w = noise texture scale
	float3    _GS_SnowData6;    // x = alpha, y = smoothness, z = altitude blending
    half4     _GS_SnowTint;

	#define SNOW_TEXTURE_SCALE _GS_SnowData5.w
	#define SNOW_NORMALS_STRENGTH _GS_SnowData4.z

	float     _GS_ReliefFlag;	// default value is 1 which will force ignore relief shading except on terrain where value is 0.1
...
//		float3 wsNormal = WorldNormalVector (IN, o.Normal);
		...
		if (snowCover * _GS_ReliefFlag > 0.1) {
			// 3D surface mapping
			float snowHeight = _GS_SnowData1.x; // Range: 0.001 .. 0.3 or will distort
			...
			for (int j=0;j<RELIEF_BINARY_SAMPLE_COUNT;j++) {
				...
				float height2 = tex2D(_GS_NoiseTex, (snowPos.xz + (_GS_SunDir.xz) * 0.03) * SNOW_TEXTURE_SCALE).x;
