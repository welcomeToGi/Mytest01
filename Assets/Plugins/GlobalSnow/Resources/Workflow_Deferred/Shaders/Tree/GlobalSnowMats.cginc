	...
	float4    _GS_SnowData4;    // x = footprint scale, y = footprint obscurance, z = internally used, w = minimum altitude for terrain including scattering
	...
	#endif
	{
		fixed snowCover = 1.0;
    	    ...
    	    #ifdef GLOBALSNOW_MOVING_OBJECT
    			snowCover = any(IN.wposOffset.xz); // animated objects will always be snow-covered on top to prevent artifacts
	    		if (!snowCover) {
	    	#endif
		    ...
		    float mask = tex2D(_GS_DepthMask, maskUV).r;
		...
		snowCover = snowCover * minAltitude * flatSurface;
