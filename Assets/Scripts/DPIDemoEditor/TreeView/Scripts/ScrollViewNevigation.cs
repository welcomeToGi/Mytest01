...
public class ScrollViewNevigation : MonoBehaviour
{
    ...
    public void Nevigate(RectTransform item)
    {
        ...
        DOTween.To(() => scrollRect.GetComponent<ScrollRect>().normalizedPosition, x => scrollRect.GetComponent<ScrollRect>().normalizedPosition = x, newNormalizedPosition, 0.8f).OnComplete(
            () =>
            {
                var v = content.GetComponent<RectTransform>().localPosition;
                v.x = (float)Math.Round(v.x, 0);
                v.y = (float)Math.Round(v.y, 0);
                content.GetComponent<RectTransform>().localPosition = v;
            });
    }
